...
I have selected this paper for my code review because I intend to implement their analysis for my own data (both in my dissertation and for the Scripting Class Project). Briefly, my overarching goal is to quantify the differences in telomere lengths on the sex chromosomes in males and females, and investigate the effect treatment (age in CH 1 and heat stress in CH 3) may have on telomere length. In a similar sequencing method, I have obtained selective seqeuncing Nanopore MinION data for the brown anole lizard and in the future will also be applying this method to the zebra finch genome. 
...
The implementation of TeloPeakCounter is missing details. There is no clear suggestion to git clone the repository. The first statement in the usage section is "TeloPeakCounter reads in fast5 data, which can be extracted using SquigglePull." That is the only statement referring to SquigglePull, meaning the authors neglect to clarify that this is not one of their own functions. There is no citation or Github provided for the original development of SquigglePull. Additionally, this statement refers to the input as the raw Nanopore sequencing output file format of .fast5 because that is what SquigglePull is solely compatible with; however, Nanopore introduced the .pod5 raw data file format that replaced .fast5 file format prior to the publication of this paper and repository. The python code for TeloPeakCounter is modularized, but lacks docstrings entirely. 

Altogether, the lack of explanation in this repository makes for obtaining, installing, and running the code to be quite difficult. I, personally, ran into road blocks when first orienting on where to start with the code in terms of order to run the functions and creating an environment with the necessary packages/applications. The lack of sanity checks further complicated troubleshooting, and I think the addition of these to the code by the authors would aid in user ability. Once I dealt with the prior issues, I really struggled with what to do with .pod5 input file format. In order to make telomere profiling accessible, I suggest that the authors update their code to work with .pod5 file format. Two ways that I have started to play around with are by 1) converting .pod5 to .fast5 for SquigglePull input or 2) directly extracting the information needed from the raw data in a .tsv output file that mimicks SquigglePull for direct input into TeloPeakCounter.
